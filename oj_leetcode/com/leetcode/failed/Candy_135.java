	...
	public int candy_others(int[] ratings) {
        ...
        for(int i=1;i<len;i++){
            if(ratings[i]<ratings[i-1]){
                ...
                prev = 1;       //The last child get 1 candy to make the number minimum.
                i--;            //since while loop take i to the next non-decreasing sequence, fix i.
            }else if(ratings[i]==ratings[i-1]){
                sum+=1;
                prev = 1;       //if equals, we give the child 1 candy, since it doesn't violate the rule.
            }else if(ratings[i]>ratings[i-1]){
